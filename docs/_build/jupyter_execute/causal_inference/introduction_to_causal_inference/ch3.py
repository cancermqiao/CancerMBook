...
# 
# ```{admonition} Assumption 3.1 (Local Markov Assumption)
# :class: note
# Given its parents in the DAG, a node $X$ is independent of all its non-descendants.
# ```
# 
# ```{admonition} Deﬁnition 3.1 (Bayesian Network Factorization)
# :class: warning
...
# ```
# 
# ```{admonition} Assumption 3.2 (Minimality Assumption)
# :class: note
...
# ## 3.3 Causal Graphs
# 
# ```{admonition} Deﬁnition 3.2 (What is a cause?)
# :class: warning
# A variable $X$ is said to be a cause of a variable $Y$ if $Y$ can change in response to changes in $X$.
# ```
# 
# ```{admonition} Assumption 3.3 ((Strict) Causal Edges Assumption)
# :class: note
...
# ## 3.7 d-separation
# 
# ```{admonition} Deﬁnition 3.3 (blocked path)
# :class: warning
...
# ```
# ```{admonition} Deﬁnition 3.4 (d-separation)
# :class: warning
...
# ```
# 
# ```{admonition} Theorem 3.1 (global Markov assumption)
# :class: important
# Given that $P$ is *Markov* with respect to $G$ (satisﬁes the local Markov assumption, Assumption 3.1), if $X$ and $Y$ are d-separated in  $G$conditioned on $Z$, then $X$ and $Y$ are independent in $P$ conditioned on $Z$. We can write this succinctly as follows:
# 
